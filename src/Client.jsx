...
const Client = () => {
  ...
  return (
    <div className="text-white min-h-screen pt-32 pb-40 selection:bg-cyan-400 selection:text-black">
      ...
      {/* Industrial Logo Wall — Infinite Roller */}
      <section className="py-48 bg-white text-black observe-section overflow-hidden mt-20">
        <div className="max-w-[1400px] mx-auto text-center mb-32 px-6">
        ...
        {/* Marquee roller */}
        <div className="relative w-full border-y border-black/10 overflow-hidden">
          <div className="flex w-max animate-marquee">
            {[...Array(3)].map((_, repeat) =>
              ["QUANTUM", "NEXUS", "FORGE", "BIOTEK", "ORBIT", "SYNTH"].map(name => (
                <div
                  key={`${repeat}-${name}`}
                  className="flex flex-col items-center justify-center px-20 py-16 border-r border-black/10 group hover:bg-black transition-all cursor-default min-w-[220px]"
                >
                  <span className="text-2xl font-black tracking-[0.3em] uppercase group-hover:text-cyan-400 transition-colors italic whitespace-nowrap">{name}</span>
                  <div className="text-[10px] font-black opacity-20 uppercase mt-4 group-hover:opacity-60 transition-opacity tracking-widest">LEVEL 1 PARTNER</div>
                </div>
              ))
            )}
          </div>
